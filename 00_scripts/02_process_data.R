separate_bike_model <-
  function(data, keep_model_column = TRUE, append = TRUE) {
    box::use(
      dplyr[mutate, select],
      stringr[str_c, str_detect, str_to_lower],
      tidyr[separate]
    )
    ...
    output_tbl <- data %>%
      ...
      mutate(
        model_base = case_when(
          ...
          # Fix Scalpel 29
          str_detect(str_to_lower(model_2), "29") ~
            str_c(model_1, model_2, sep = " "),
      ...
      mutate(
        model_tier = model %>%
          str_replace(model_base, replacement = "") %>%
          str_trim()
      ) %>%
...
separate_bike_description <-
  function(data, keep_description_column = TRUE, append = TRUE) {
    ...
    if (!keep_description_column) {
      output_tbl <- output_tbl %>% select(-description)
    }
